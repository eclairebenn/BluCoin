...
server.post("/tickers/", function (req, res, next) {
  const error = validateTicker(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new tickers.
    next();
  }
});
...
function validateTicker(ticker) {
  if (!ticker.ticker) return "Ticker is required.";
  if (!ticker.name) return "Name is required.";
  if (!ticker.base_currency_name) return "Base_currency_name is required.";
  return "";
}
